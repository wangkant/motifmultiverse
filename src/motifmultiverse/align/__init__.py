...
class AlignmentEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        # Bounds come from schema.EVIDENCE_FIELD_DOMAINS rather than from literals
        # here, because schema.criteria resolves `provenance: derived` thresholds
        # against that same table. Two copies of these intervals -- one enforced,
        # one consulted -- could drift, and a drifted copy would let a threshold
        # claim to be a field's own boundary while the field was validated into a
        # different range.
        def in_domain(name: str, value: Any) -> None:
            low, high = EVIDENCE_FIELD_DOMAINS[name]
            finite_measure(name, value, low, high)

        in_domain("overlap_frac_source", self.overlap_frac_source)
        in_domain("overlap_frac_target", self.overlap_frac_target)
        in_domain("ppm_similarity", self.ppm_similarity)
        if self.signed_cwm_similarity is not None:
            in_domain("signed_cwm_similarity", self.signed_cwm_similarity)
        if self.null_shuffles == 0:
            ...
            in_domain("empirical_p_value", self.empirical_p_value)
