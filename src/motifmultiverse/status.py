...
def render_markdown(status: dict[str, Any]) -> str:
    ...
    The scope sentence and the evidence column are generated too, and that is the
    point of both. `BackendProbe.capability` is rendered beside every backend row
    so nobody has to open the source to learn what that green covers; the module
    table used to make a similarly-shaped claim with none of that discipline --
    the word IMPLEMENTED alone, with `module_status`'s `detail` (the runner the
    subcommand dispatches to, which is the evidence the derivation ran on)
    dropped on the floor, and the definition left in hand-written prose *outside*
    the markers. A block delimited so it can be regenerated on its own also
    travels on its own: into `PKG-INFO`, onto the project page, into a quotation.
    A claim whose scope can be separated from it will be read without its scope.
    """
    lines = [
        BEGIN_MARKER,
    ...
    for name, entry in status["modules"].items():
        # The evidence, rendered as code only when it is a symbol: a SKELETON's
        # detail is a sentence naming the module README, not a dotted path.
        detail = entry["detail"]
        evidence = f"`{detail}`" if entry["status"] == "IMPLEMENTED" else detail
        lines.append(f"| `{name}` | {entry['status']} | {evidence} |")
    lines += [
        "",
