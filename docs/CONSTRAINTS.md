...
| `FP-22` | Four-state missingness needs an executable assertion, not a line in a principles table; undefined values take an explicit sentinel; coverage is computed before any fill. | `ENFORCED` | `schema.Missingness` + `schema.MISSING_SENTINEL`, which run on every hit row read; `guards.four_state_missingness` has **no call site** (`guards.GUARDS_AWAITING_INPUT`) -- no artifact claims a coverage independently of the code that would recompute it -- so this row rests on the schema half | — |
