...
> A guard nothing calls does not satisfy this: an artifact that violates the clause
> still comes out of this tool unchallenged, so the row rests on a check that never
> meets the data. `guards.GUARDS_AWAITING_INPUT` names those guards, and
> `test_no_enforced_row_rests_only_on_a_guard_with_no_call_site` refuses the label
> to any row whose `enforced_by` names nothing else.
...
It then went to **2/15/8** without a line of analysis code changing. `FP-12` and
`FP-21` were labelled `ENFORCED` on the strength of `guards.estimability_floor` and
`guards.interaction_required`, two guards that `guards.GUARDS_AWAITING_INPUT` itself
records as having no call site — so no artifact this tool emits was ever put in front
of either. The number can fall when a label is corrected, and a roadmap metric that
only ever rises is measuring the labeller rather than the code.
...
| `FP-02` | Merge promoter+distal *within* a model/readout; never average CWMs across models. | `PARTIAL` | nothing in this release combines CWMs at all, and that is now checked rather than assumed: `compile.operations_log` reads each compiled lexicon back and classifies every emitted motif against the registry arrays it stands for, and `guards.no_cross_model_cwm_avg` refuses the compile before publication if any of them is a mean that does not hold model, readout and metacluster fixed. What the check reaches is bounded, and the bound is pinned by a test rather than described: its reference set is the registry, so a cross-model mean performed *before* `ingest` — where a meta-analysed-CWM stage would live — arrives as an ordinary registry motif, is classified `copy`, and **passes**. An average over inputs whose matrices are identical is not in the bytes at all. The within-analysis union is `compile/`, a skeleton | — |
...
| `FP-12` | Compute the affected peak count before deciding; an estimate below the floor, or an interval containing both zero and the reference, is `NOT_ESTIMABLE_UNDERPOWERED` with no direction. | `PARTIAL` | `validate.evaluate_stability` computes the affected peak count before any decision and `StabilityResult` refuses a result below 30 affected peaks that is not `LOW_RISK_RARE_NOT_VALIDATED`, which mechanises the count-before-deciding clause; the estimability clause is enforced nowhere — `guards.estimability_floor` is defined and falsification-tested but has **no call site** (`guards.GUARDS_AWAITING_INPUT`), because no stage emits stratified cells carrying an N and a CI against a reference | *(remaining clause)* Every emitted effect cell carries its N and, where it reports an interval, the reference that interval is read against; a cell whose N is below the preregistered floor, or whose interval contains both zero and that reference, carries `NOT_ESTIMABLE_UNDERPOWERED` and no direction. **FAIL** on a cell with no N, on an under-floor cell not marked `NOT_ESTIMABLE_UNDERPOWERED`, or on a direction read off an interval containing both zero and the reference. |
...
| `FP-19` | Trimmed-core length ≤ 6 bp, TomTom *q* > 0.05 or seqlet count < 100 ⇒ flagged, shown only by `variant_id`, reported beside its chance expectation; a single-family layer's share is `NOT_ESTIMABLE`. | `PARTIAL` | `guards.short_motif_flag` at `annotate`, reading the TRIMMED CORE. Which length was left implicit while only one existed; on real tfmodisco-lite output `motif_length` is the padded pattern window and is 50 on all 139 nodes of the case study, so a ≤6bp clause reading it could never fire, while 40 of those 139 have a contribution-bearing core ≤ 6 bp. The padded window is still recorded, it is just not the width the rule reads; the single-family share clause is enforced nowhere -- `guards.single_family_layer` has **no call site** (`guards.GUARDS_AWAITING_INPUT`), because `interpret.FamilyComposition.peak_share` counts peaks in its denominator rather than the family layer, so its 1.0 is not the undefined share the guard is about; the two reporting clauses (variant_id-only display, chance expectation alongside) are unchecked | — |
...
| `FP-21` | Specificity is licensed by an interaction interval excluding zero, never by a difference of two significance statuses. | `DOC_ONLY` | nothing runs. `guards.interaction_required` is defined and falsification-tested but has **no call site** (`guards.GUARDS_AWAITING_INPUT`): this release emits per-family effects, not interactions, so no artifact carries a specificity claim for it to read | Every emitted specificity or between-group claim carries an interaction estimate with an interval, that interval excludes zero, and the claim does not name a difference of two significance statuses as its derivation. **FAIL** if a specificity claim carries no interaction estimate, if its interval contains zero, or if it is derived from two significance statuses. |
| `FP-22` | Four-state missingness needs an executable assertion, not a line in a principles table; undefined values take an explicit sentinel; coverage is computed before any fill. | `ENFORCED` | `schema.Missingness` + `schema.MISSING_SENTINEL`, which run on every hit row read; and `guards.four_state_missingness` now **has a call site** -- `interpret.verify_missingness_against_ledger` recomputes `defined`/`total`/coverage from the raw missingness column and refuses a disagreement with the `substrate.OpportunityLedger` written by the program that FROZE the run. That is what took the row off the schema half alone: the claim now comes from outside this package, so the guard can fail for the reason it exists. It runs only where a ledger is supplied (`--opportunity-ledger`); a run without one is unchecked on this axis and does not pretend otherwise | — |
...
| `FP-24` | A stratifying variable entering an interaction uses one definition for all units; a different definition is a named sensitivity check; the cross-tabulation comes before any effect is visible. | `PARTIAL` | `guards.stratum_parity` states the one-rule clause but has **no call site** (`guards.GUARDS_AWAITING_INPUT`), since no stage emits strata; the ordering clause is unchecked (`guards.health_before_effect` covers the health numbers, not the stratum table) | — |
...
None of the thirteen drafted this way had to be recorded `NOT_YET_DECIDABLE`.
