...
| `FP-02` | Merge promoter+distal *within* a model/readout; never average CWMs across models. | `PARTIAL` | the cross-model prohibition is structural rather than checked: nothing in this release combines CWMs at all, and `guards.no_cross_model_cwm_avg` has **no call site** (`guards.GUARDS_AWAITING_INPUT`) because no stage logs a CWM combination for it to read; the within-analysis union is `compile/`, a skeleton | — |
