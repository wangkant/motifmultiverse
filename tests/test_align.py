...
# ------------------------------------------------- the speedup must move no number
#
# The five tests below exist because this stage is expensive (9,591 pairs x 1000
# re-registrations on the real case study) and will therefore keep attracting
# optimisation. Each one pins a different way an optimisation could go wrong, and
# together they say: the output is reproducible, its numbers are reproducible
# from an INDEPENDENT formula rather than from the implementation being tested,
# and the two things that were hoisted out of the null loop are provably things a
# shuffle cannot reach.


def test_align_registry_is_byte_reproducible_run_to_run(tmp_path):
    """Same registry, same seed, twice -- the same bytes in every artifact.

    This is the acceptance property for any performance work on this stage,
    written down as a test so it is checked on every commit instead of once by
    whoever did the optimising. A faster null that moves a p-value is not a
    faster analysis, it is a different one, and the difference would reach
    `evidence/` as four files that look exactly as authoritative as the old ones.

    Run to run rather than only before/after a change, because that is the form
    a future defect actually takes: a cache keyed on array contents, a plan
    accidentally shared between pairs, a generator hoisted to module level, an
    `lru_cache` whose hit rate depends on call order. None of those change the
    arithmetic of a single registration -- they make the SECOND run of the same
    registry differ from the first, or the parallel run differ from the serial
    one, which is why this and
    `test_align_registry_writes_byte_identical_tables_at_every_worker_count` are
    both here and neither replaces the other.
    """
    registry = _parallel_registry(tmp_path)
    first, second = tmp_path / "first", tmp_path / "second"

    first_summary, first_edges = align_registry(
        registry, first, null_shuffles=25, seed=11, workers=1)
    second_summary, second_edges = align_registry(
        registry, second, null_shuffles=25, seed=11, workers=1)

    assert first_summary.n_edges > 1, "fixture registers too few pairs to be a pin"
    assert first_edges == second_edges
    for name in ALIGN_ARTIFACTS:
        assert (first / name).read_bytes() == (second / name).read_bytes(), (
            f"{name} differs between two runs of the same registry at the same "
            "seed, so some state outside the seed and the pair is reaching the "
            "numbers"
        )
...
@pytest.mark.parametrize(
    ...
    [(6, 6), (7, 7), (8, 8), (11, 12), (12, 11), (12, 12), (9, 17), (30, 29)],
)
def test_align_null_is_bit_identical_to_registering_each_shuffle_from_scratch(
    source_len, target_len,
):
    """The hoist must be a hoist: same numbers, computed sooner.

    `calibrate_pair_null` builds the shuffle-independent half of the search once
    per pair and reuses it across every shuffle. This recomputes the same null
    the slow way -- one full `register_pair` per permutation, each building its
    own candidate list and its own source norms from nothing -- and requires the
    two to agree as float64 BIT PATTERNS, not to some tolerance. A last-ulp
    difference is exactly what a reordered reduction (a prefix-sum norm, a
    batched gemm, a reverse-complement symmetry) produces, and it is enough to
    flip `score > best[0]` and select a different offset.

    Swept over core-length combinations rather than one, because how many
    candidate windows clear the floor -- and therefore how much is hoisted --
    depends on both lengths, and the equal-length case is the easy one.
    """
    from motifmultiverse.align import calibrate_pair_null, register_pair

    source = _informative_core(source_len * 7 + 1, source_len)
    target = _informative_core(target_len * 13 + 2, target_len)
    seed, shuffles = 4, 40

    p_value, null_scores = calibrate_pair_null(
        source, target, null_shuffles=shuffles, seed=seed)
    ...
    assert len(null_scores) == shuffles
    assert [s.hex() for s in null_scores] == [s.hex() for s in from_scratch], (
        "the hoisted per-pair search plan produced a different null from "
        "re-deriving everything per shuffle, so it is not a hoist"
    )
    n_extreme = sum(1 for s in from_scratch if s >= register_pair(source, target).ppm_similarity)
    assert p_value == (n_extreme + 1) / (shuffles + 1)


def test_align_norm_flat_is_bit_identical_to_numpy_linalg_norm():
    """`_norm_flat` is `np.linalg.norm`'s own body, not an approximation of it.

    The module replaced `float(np.linalg.norm(v))` with `math.sqrt(v.dot(v))`
    because numpy's `ord=None` fast path IS `sqrt(x.ravel(order='K').dot(x))`.
    That is a claim about a numpy internal, so it is checked here rather than
    trusted: if a future numpy changes that fast path -- adds the rescaling the
    old `_cosine` docstring wrongly believed was already there -- this fails
    loudly instead of silently shifting every score in the table by an ulp.

    Compared as raw bit patterns, over the magnitudes where the two could
    plausibly diverge: the overflow-to-inf range the `_cosine` fallback exists
    for, the underflow-to-zero range below it, and subnormals.
    """
    import struct

    from motifmultiverse.align import _norm_flat
    ...
    vectors = []
    for scale in (5e-324, 1e-320, 1e-170, 1e-8, 1.0, 1e8, 1e155, 1e160, 1e300, 1e308):
        for length in (4, 16, 24, 48, 120, 196):
            vectors.append(rng.random(length) * scale)
            vectors.append((rng.random(length) - 0.5) * scale)
    vectors += [np.zeros(24), np.array([0.0, -0.0]), np.full(8, np.inf),
                np.full(8, -np.inf), np.array([np.inf, 1.0]), np.array([1e308] * 4)]

    with np.errstate(over="ignore", invalid="ignore"):
        for v in vectors:
            v = np.ascontiguousarray(v, dtype=float)
            assert bits(_norm_flat(v)) == bits(float(np.linalg.norm(v))), (
                f"_norm_flat diverged from np.linalg.norm on a {v.size}-element "
                f"vector at scale {abs(v).max():g}"
            )


def test_align_admissible_windows_cannot_see_a_shuffle_and_applies_the_floor():
    """Why hoisting the candidate list out of the null loop is safe at all.

    Two properties, and the hoist needs both. First, `_admissible_windows` is a
    function of the two LENGTHS and the two floor parameters -- it never receives
    a matrix, so a row permutation of the target (which is what a null draw is)
    cannot reach it. That is not an optimisation argument, it is the reason all
    ~1000 of a pair's null registrations were re-deriving one identical list.

    Second, the floor is applied while the list is BUILT, so an inadmissible
    window is absent from candidacy rather than scored and rejected afterwards.
    That is the guarantee the module docstring is written about -- a 3-4bp window
    that reaches a chance-perfect cosine must never enter the comparison -- and
    moving the floor into a helper is precisely the edit that could quietly
    demote it to a post-filter.
    """
    from motifmultiverse.align import _admissible_windows, _candidate_windows

    windows = _admissible_windows(12, 11, DEFAULT_MIN_OVERLAP_BP, DEFAULT_MIN_OVERLAP_FRAC)
    assert windows, "fixture lengths should admit at least one window"

    # Every emitted window clears BOTH halves of the bilateral floor...
    for _offset, _s0, _s1, _t0, _t1, overlap_bp, frac_source, frac_target in windows:
        assert overlap_bp >= DEFAULT_MIN_OVERLAP_BP
        assert frac_source >= DEFAULT_MIN_OVERLAP_FRAC
        assert frac_target >= DEFAULT_MIN_OVERLAP_FRAC
    # ...and every window that does NOT is absent, not merely ranked last.
    admissible_offsets = {w[0] for w in windows}
    for offset, _s0, _s1, _t0, _t1, overlap_bp in _candidate_windows(12, 11):
        clears = (overlap_bp >= DEFAULT_MIN_OVERLAP_BP
                  and overlap_bp / 12 >= DEFAULT_MIN_OVERLAP_FRAC
                  and overlap_bp / 11 >= DEFAULT_MIN_OVERLAP_FRAC)
        assert (offset in admissible_offsets) is clears
    # Ascending offset order, which is what makes the strict `>` tie-break
    # ("first equal score wins") a stable, reportable choice.
    assert [w[0] for w in windows] == sorted(w[0] for w in windows)
    # A pair whose cores cannot bilaterally overlap admits nothing at all --
    # register_pair's refusal, one level down.
    assert _admissible_windows(4, 4, DEFAULT_MIN_OVERLAP_BP, DEFAULT_MIN_OVERLAP_FRAC) == ()


def test_align_refuses_a_search_plan_built_for_a_different_pair():
    """The private hoist argument fails loudly rather than answering wrongly.

    `_prepared_source` is an internal fast path, and the failure mode of any such
    argument is that someone later reuses a plan across pairs "because the
    lengths match" and gets a p-value computed against the wrong source. The plan
    carries what it was built for and `register_pair` checks it, so that mistake
    raises instead of producing a number.
    """
    from motifmultiverse.align import _prepare_source, register_pair

    source = _informative_core(1, 12)
    other_source = _informative_core(2, 12)
    target = _informative_core(3, 12)

    plan = _prepare_source(source, target.shape[0],
                           DEFAULT_MIN_OVERLAP_BP, DEFAULT_MIN_OVERLAP_FRAC)
    # Its own pair still works, and agrees with the unhoisted call bit for bit.
    hoisted = register_pair(source, target, _prepared_source=plan)
    plain = register_pair(source, target)
    assert hoisted.ppm_similarity.hex() == plain.ppm_similarity.hex()
    assert (hoisted.offset, hoisted.orientation) == (plain.offset, plain.orientation)

    # A different source matrix of the SAME length: the case a length check alone
    # would wave through.
    with pytest.raises(AlignmentError, match="_prepared_source"):
        register_pair(other_source, target, _prepared_source=plan)
    # A different target length.
    with pytest.raises(AlignmentError, match="_prepared_source"):
        register_pair(source, _informative_core(4, 11), _prepared_source=plan)
    # A different overlap floor -- the plan was filtered with the old one.
    with pytest.raises(AlignmentError, match="_prepared_source"):
        register_pair(source, target, min_overlap_bp=8, _prepared_source=plan)
